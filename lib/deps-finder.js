...
export default class DepsFinder {
  ...
  async resolveAndIncludePackageJson(fromPath, moduleSpecifier) {
    const resolvedFiles = [(await this.resolveFrom(fromPath, moduleSpecifier)) || moduleSpecifier];
    ...
    if (pathParts.length === 1) {
      resolvedFiles.unshift(
          await this.resolveFrom(`${this.#baseDirectory}/package.json`, `${moduleSpecifier}/package.json`));
  ...
  async findDeps(filepath, ast) {
    ...
    for (let dep of deps) {
      depsPlusPackageJsonFiles.push(...await this.resolveAndIncludePackageJson(filepath, dep));
    }

    const resolvedChildChunks = [];
    for (let childChunkFilepath of childChunks) {
      const resolvedFiles = await this.resolveAndIncludePackageJson(filepath, childChunkFilepath);
      // The package.json must be included in the compilation, but it doesn't matter where.
      // We want to avoid treating it as a split point, so just include it in the current module deps
      if (resolvedFiles.length > 1) {
        depsPlusPackageJsonFiles.push(resolvedFiles[0]);
        resolvedChildChunks.push(...resolvedFiles.slice(1));
      } else {
        resolvedChildChunks.push(...resolvedFiles);
      }
    }
