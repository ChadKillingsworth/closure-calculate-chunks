...
function findDepsFromEntryPoints(
  ...
  entrypoints.forEach((entrypoint, index) => {
    graph.setNode(entrypoint, new GraphNode(entrypoint));
    if (index === 0) {
  ...
  while (entrypoints.length > 0) {
    ...
    const {deps, childChunks} = getDependenciesForFile(
        entrypoint, fileDepsCache, baseDirectory, googBasePath, googDepsMap, dependenciesToHoist);
    deps.forEach(dep => {
    ...
    if (manualEntrypoint) {
      ...
      graph.setNode(child, new GraphNode(child, new Set(), new Set(), new Set([child])));
