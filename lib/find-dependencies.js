...
function getDependenciesForFile(
  ...
  if (cachedDeps) {
    ...
    childChunkRefs = cachedDeps.childChunkRefs;
    parentChunkRefs = cachedDeps.parentChunkRefs;
  } else {
    if (!filepath.endsWith('.json')) {
      ...
      try {
        ...
        childChunkRefs = depInfo.childChunkRefs.slice();
        parentChunkRefs = depInfo.parentChunkRefs.slice();
      } catch (e) {
  ...
  for (let i = 0; i < parsedDeps.length; i++) {
    ...
    const transientDepInfo =
        getDependenciesForFile(graph, parsedDeps[i], cache, baseDirectory, googBasePath, googDepsMap, visitedFiles);
