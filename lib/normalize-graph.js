...
function lowestCommonAncestor(entrypoint, sourceNodes, graph, nodeDistanceFromEntrypoint) {
  ...
  allPaths = allPaths.filter(pathSet => pathSet.has(entrypoint));
...
function normalizeGraph(entrypoint, graph) {
  ...
  // Ensure every node has a path to the entrypoint
  Object.keys(nodeDistanceFromEntrypoint)
      .filter(nodeName => !Number.isFinite(nodeDistanceFromEntrypoint[nodeName].distance))
      .forEach(nodeName => {
        const dependentNodes = graph.outEdges(nodeName).map(edge => edge.w);
        if (dependentNodes.length > 0) {
          const lca = lowestCommonAncestor(entrypoint, dependentNodes, graph, nodeDistanceFromEntrypoint);
          graph.setEdge(lca, nodeName);
        }
      });
