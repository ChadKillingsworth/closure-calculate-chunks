...
export default function normalizeGraph(entrypoint, graph) {
  const allPackageJsonFiles = new Set();
  graph.nodes().forEach((nodeName) => {
    const node = graph.node(nodeName);
    node.sources = new Set(Array.from(node.deps));
    node.packageJsonFiles.forEach((packageJsonFile) => allPackageJsonFiles.add(packageJsonFile));
  });
  const entrypointNode = graph.node(entrypoint);
  entrypointNode.sources = new Set(Array.from(allPackageJsonFiles).concat(Array.from(entrypointNode.sources)));
